...
- automatic 7-day Solo trial for each new user's initial workspace, with the full 1,500-request plan allowance and no card or Stripe subscription required
...
The initial workspace trial is created atomically with the new account, uses the first commercial plan in the public catalog (currently Solo), and ends exactly seven days later. It creates no Stripe customer, subscription, or payment method. After expiry, Forge preserves read-only history and safe deactivation/deletion actions but returns `402 SUBSCRIPTION_REQUIRED` for product writes and executions until the owner completes Checkout. Checkout always collects a payment method and creates a paid subscription without an additional provider trial; redirects never grant product access. The implementation has no automatic overage: limits stop execution until renewal or an explicit plan change.
