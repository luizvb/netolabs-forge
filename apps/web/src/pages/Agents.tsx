...
export function NewAgent({ guest = false }: { guest?: boolean }) {
  ...
  useEffect(() => {
    if (guest) sessionStorage.setItem(PENDING_DRAFT_KEY, JSON.stringify({ name, description, definition, guardrails: guardrailInput, instructions, model } satisfies AgentDraft));
    else sessionStorage.removeItem(PENDING_DRAFT_KEY);
  }, [guest, name, description, definition, guardrailInput, instructions, model]);
  ...
  const generate = async () => {
    ...
    try {
      if (guest) {
        const result = buildDraftPrompt({ name, description, definition, guardrails: guardrailInput, instructions, model });
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (guest) {
        sessionStorage.setItem(PENDING_DRAFT_KEY, JSON.stringify({ name, description, definition, guardrails: guardrailInput, instructions, model } satisfies AgentDraft));
  ...
  return <Page title="Novo agente" subtitle={guest ? 'Crie e revise agora. Entre apenas quando estiver pronto para publicar.' : 'Comece pela intenção. O Forge estrutura papel, limites, grounding e escalonamento.'} backTo={guest ? '/' : '/agents'} wide>
    <form onSubmit={submit} className="prompt-builder">
      ...
      <section className="builder-prompt"><div className="builder-section-head"><div><h2>Prompt oficial</h2><p>Revise o contrato antes de publicar o agente.</p></div><ShieldCheck size={21}/></div><Textarea label="Instruções do sistema" value={instructions} onChange={(event) => setInstructions(event.target.value)} rows={28} required minLength={20} placeholder="Gere o prompt a partir da definição ou escreva manualmente."/>{generated && <div className="prompt-meta"><span>{generated.guardrails.length} guardrails</span><span>{generated.source === 'google-adk' ? generated.model : 'template Forge'}</span><span>{guest ? 'rascunho local' : `${generated.usage.totalTokens} tokens de geração`}</span></div>}<div className="builder-actions"><Button type="button" variant="ghost" disabled={!instructions} onClick={() => void navigator.clipboard.writeText(instructions)}><Copy size={16}/>Copiar</Button><Button disabled={saving || !instructions.trim()}>{saving ? 'Aguarde...' : guest ? 'Publicar agente' : 'Criar agente'}</Button></div>{guest && <small className="publish-helper">O login será solicitado somente na publicação. Seu rascunho será preservado.</small>}</section>
    </form>
  </Page>;
}
