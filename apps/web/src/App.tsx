import { ArrowSquareIn, ChartLineUp, CreditCard, Gauge, Robot, SignOut } from '@phosphor-icons/react';
...
function Shell({ children, user, onLogout, guest = false }: { children: ReactNode; user?: { name: string }; onLogout?: () => void; guest?: boolean }) {
  ...
  const nav = guest ? [['/agents/new', 'Agentes', Robot]] as const : [['/', 'Painel', Gauge], ['/agents', 'Agentes', Robot], ['/observability', 'Observabilidade', ChartLineUp], ['/billing', 'Plano', CreditCard]] as const;
...
export function App() {
  ...
  return <Shell user={user!} onLogout={logout}><Routes><Route path="/" element={<Dashboard/>}/><Route path="/agents" element={<AgentsPage/>}/><Route path="/agents/new" element={<NewAgent/>}/><Route path="/agents/:id" element={<AgentDetail/>}/><Route path="/observability" element={<Observability/>}/><Route path="/billing" element={<Billing/>}/><Route path="*" element={<Navigate to="/" replace/>}/></Routes></Shell>;
}
